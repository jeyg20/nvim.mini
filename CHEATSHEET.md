...
| `]t` / `[t` is **tags**, not tabs | From Neovim defaults (`:tnext`). To change _tab_ use `gt`/`gT`; to change _buffer_ use `]b`/`[b`.                        |
...
| Keys                      | Action                                           | Source           |
| ------------------------- | ------------------------------------------------ | ---------------- |
| `j` / `k`                 | Move by _screen_ line when no count (wrap-aware) | mini.basics      |
...
> `rw` and `rp` are plain text substitution for filetypes with no LSP — they _will_ hit
